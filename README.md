...
- Explicit **Check exposure** button, plus an optional "check as I type" mode
...
- Works over `file://` too, falling back to a pure JS SHA-1 when WebCrypto is
...
- SHA-1 is not a defence. It is fast and unsalted; a leaked hash of a
  password a human chose is a cracked password, usually within seconds.
...
hash starting with `CBFDA`." The server returns roughly 800 to 3,000 of them. It
...
1. **The server learns almost nothing.** A prefix of 5 hex characters is 20
   bits. Against a corpus of ~850 million hashes, that identifies a set of
   thousands, not an individual.
...
k-anonymity is a *design* that makes the policy unnecessary. That is the
...
**Why serve it rather than double clicking `index.html`?** Both work, but
...
├── js/
│   ├── sha1.js              # WebCrypto, falling back to pure JS for RFC 3174
...
variable local to the module, never written to `localStorage`, a cookie, or
the URL, and is wiped after each request.
...
## Pentest methodology and OWASP WSTG
...
weak credentials. Modern guidance, including **NIST SP 800-63B §5.1.1.2**,
...
this surfaces in three places: checking whether registration and password
reset flows screen against breach corpora, testing whether an account that's
in scope can be reached by spraying breached credentials from the target's own
domain, and reporting reuse between a breached personal account and a
corporate one.
...
Password data from [Have I Been Pwned's Pwned
