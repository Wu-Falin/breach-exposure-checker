 ...
 *   - The key is held in a module-local variable only. It is not written to
 ...
 *   `hibp-api-key` and `user-agent` headers server-side. A ~20 line example is
...
(function (global) {
  ...
  async function checkEmail(email, baseUrl) {
    ...
    // (`user-agent` is a forbidden header in browsers — the proxy must add it.)
    if (hasApiKey()) {
